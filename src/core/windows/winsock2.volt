...
enum int FIONBIO =       cast(int)-2147195266;//(IOC_IN | ((cast(i32)typeid(u32).size & IOCPARM_MASK) << 16) | (102 << 8) | 126);
...
fd_set* FD_CREATE(uint capacity)
{
    ...
    size_t size = typeid(fd_set_one).size - typeid(SOCKET).size + (typeid(SOCKET).size * capacity);
