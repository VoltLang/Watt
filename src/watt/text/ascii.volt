...
//! Returns true if @p c is alphanumeric (a-z,A-Z,0-9).
fn isAlphaNum(c: dchar) bool
{
	return isAlpha(c) || isDigit(c);
}

//! Returns true if @p c is alphabetical (a-z,A-Z).
fn isAlpha(c: dchar) bool
{
	return isLower(c) || isUpper(c);
}

//! Returns true if @p c is lowercase alphabetical (a-z).
fn isLower(c: dchar) bool
{
	return c >= 'a' && c <= 'z';
}

//! Returns true if @p c is uppercase alphabetical (A-Z).
fn isUpper(c: dchar) bool
{
	return c >= 'A' && c <= 'Z';
}

//! Returns true if @p c is a digit (0-9).
fn isDigit(c: dchar) bool
{
	return c >= '0' && c <= '9';
}

//! Returns true if @p c is an octal digit (0-7).
fn isOctalDigit(c: dchar) bool
{
	return c >= '0' && c <= '7';
}

//! Returns true if @p c is a hex digit (0-9,a-f,A-F).
fn isHexDigit(c: dchar) bool
{
	return c >= '0' && c <= '9' || c >= 'a' && c <= 'f' || c >= 'A' && c <= 'F';
}

//! Returns true if @p c is whitespace.
fn isWhite(c: dchar) bool
{
	return c == ' ' || c == '\t' || c == '\r' || c == '\f' || c == '\v' || c == '\n';
}

//! Returns true if @p c is a control character.
fn isControl(c: dchar) bool
{
	return c < 32 || c == 127;
}

//! Returns true if @p c is punctuation.
fn isPunctuation(c: dchar) bool
{
	return !isAlphaNum(c) && !isWhite(c) && !isControl(c);
}

//! Returns true if @p c is a printable character.
fn isPrintable(c: dchar) bool
{
	return c >= 32 && c <= 126;
}

//! Returns true if @p c is an ASCII character.
fn isASCII(c: dchar) bool
{
	return c <= 127;
}

//! Returns @p c, or @p c as a lowercase character if it's an uppercase character.
...
//! Returns @p c, or @p c as an uppercase character if it's a lowercase character.
